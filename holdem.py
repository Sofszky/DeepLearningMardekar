...
class Poker:
    ...
    def betting(self, Choice, was_raised):
        if (Choice == 0) & (not was_raised): #if the player wants to raise (add money)
            return "Raise"
        elif (Choice == 1) & (not was_raised): #Do nothing (if nobody raised before them)
            return "Check"
        elif (Choice == 2) & was_raised : #If somebody raised, you can hold (add the same amount as the others)
            return "Hold"
        elif (Choice == 3) & was_raised: #If your cards suck, you can throw them
            return "Fold"
        else:
            return 4

    def betting_phase(self, players_hands, number_of_players):
        all_the_money = 0
        was_raised = 0
        for hand in players_hands:
            bets = []
            if was_raised == 0:
                choice = random.randint(0, 1)  # it's random for now, but this is where the agent will have to make a decision based on the rewards
            else:
                choice = random.randint(2, 3)

            bet = self.betting(choice, was_raised)
            bets.append(bet)

            if bet == "Raise":
                all_the_money = all_the_money + 5
                was_raised = 1
                ...
                print("An error has occured during the betting")
